...
namespace md_audio {
    ...
    template <std::uint16_t TAPS>
    constexpr MdFloat TapDelayLinear<TAPS>::get_max_delay() noexcept {
        return m_max_delay;
    }
    ...
    template <std::uint16_t TAPS>
    void TapDelayLinear<TAPS>::write(MdFloat in) noexcept {
        m_writer.write(in);
    ...
    template <std::uint16_t TAPS>
    MdFloat TapDelayLinear<TAPS>::read(std::uint32_t index) noexcept {
