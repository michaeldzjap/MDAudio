...
TEST_CASE("cubicly interpolated delays can delay the signal by a given amount of time", "[DelayCubic]") {
    ...
    auto delay_time = .001;
    DelayCubic<StaticAllocator<MdFloat, StaticPool<POOL_SIZE>>>::set_sample_rate(SAMPLE_RATE);
    DelayCubic<StaticAllocator<MdFloat, StaticPool<POOL_SIZE>>> delay(allocator, MAX_DELAY_TIME, delay_time);
