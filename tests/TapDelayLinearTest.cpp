...
TEST_CASE("linearly interpolated tap delays can delay the signal by a given amount of time", "[DelayLinear]") {
    ...
    TapDelayLinear<StaticAllocator<double, StaticPool<POOL_SIZE>>, TAPS>::set_sample_rate(SAMPLE_RATE);
    TapDelayLinear<StaticAllocator<double, StaticPool<POOL_SIZE>>, TAPS> delay(allocator, MAX_DELAY_TIME, delay_time);
    ...
    REQUIRE(delay.process(1.)[0] == Approx(.9)); // a + x(b - a) = 1 + 0.1(0 - 1)
    ...
    REQUIRE(delay.process(.5)[0] == Approx(.525)); // a + x(b - a) = 0.5 + 0.05(1 - 0.5)
