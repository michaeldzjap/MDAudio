...
TEST_CASE("linearly interpolated delays can delay the signal by a given amount of time", "[DelayLinear]") {
    ...
    auto delay_time = .001;
    DelayLinear<StaticAllocator<double, StaticPool<POOL_SIZE>>>::set_sample_rate(SAMPLE_RATE);
    DelayLinear<StaticAllocator<double, StaticPool<POOL_SIZE>>> delay(allocator, MAX_DELAY_TIME, delay_time);
